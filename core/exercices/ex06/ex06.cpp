...
// Camera slightly back to see the sphere
...
int main(int argc, char* argv[])
{
    ...
    GLFWwindow* window = glfwCreateWindow(width, height, "Exercise 06 - Phong Specular", nullptr, nullptr);
    if (window == NULL) {
        glfwTerminate();
    ...
    // Vertex Shader: Only calculates positions and passes them down
    const std::string sourceV = "#version 330 core\n"
        ...
        "   WorldPos = wPos.xyz; \n"
        "   WorldNormal = vec3(itM * vec4(normal, 1.0)); \n" 
        "   gl_Position = P * V * wPos; \n"
        "}\n";

    // Fragment Shader: Does all the heavy math for Phong Specular
    const std::string sourceF = "#version 330 core\n"
        ...
        "void main() { \n"
        // 1. RE-NORMALIZE the normal after interpolation!
        "   vec3 N = normalize(WorldNormal); \n"
        
        // 2. Calculate vectors
        "   vec3 L = normalize(u_light_pos - WorldPos); \n"
        "   vec3 V = normalize(u_view_pos - WorldPos); \n"
        ...
        "   vec3 specular = vec3(specularStrength * specAmount); \n"
