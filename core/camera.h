...
public:
    ...
    {
        // 1. Multiply the offsets by the camera's sensitivity
        xoffset *= MouseSensitivity;
        yoffset *= MouseSensitivity;

        // 2. Add the offsets to the camera's yaw and pitch
        Yaw   += xoffset;
        Pitch += yoffset;

        // 3. Make sure that when pitch is out of bounds, screen doesn't get flipped
        if (constrainPitch)
        {
            if (Pitch > 89.0f)
                Pitch = 89.0f;
        ...
        // 4. Update Front, Right and Up Vectors using the updated Euler angles
        updateCameraVectors();
    }
